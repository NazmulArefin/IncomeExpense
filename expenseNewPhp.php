<?php
	...
	{		
		$id = $_POST['id'];
		$queryString = 'SELECT expense_name FROM expense WHERE id_expense = '.$id;
	...
	else
	{
		...
		{
			$name = $_POST['name'];
			$queryString = "SELECT * FROM expense WHERE expense_name = '".$name."'";
			$query_res = mysqli_query($link, $queryString);
			if(mysqli_num_rows($query_res) > 0)
			{
				...
				$queryString = "INSERT INTO expense (expense_name) VALUES ('".$name."')";
				$query_res = mysqli_query($link, $queryString);
				if($query_res)
				{
		...
		{
			$id = $_POST['id'];
			$queryString = 'DELETE FROM expense WHERE id_expense = '.$id;
			$query_res = mysqli_query($link, $queryString);
			if($query_res)
			{
			...
			if(mysqli_num_rows($query_res) > 0)
			{
				...
				$queryString = "UPDATE expense SET expense_name = '".$name."' WHERE id_expense = ".$id;
				$query_res = mysqli_query($link, $queryString);
				if($query_res)
				{
